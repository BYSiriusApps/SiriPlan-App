...
export default function VeriGocuPage() {
  ...
  const getMigrateText = (key: string) => t(`dataMigrationPage.${key}`);

  const steps = [
    t("dataMigrationPage.step1"),
    t("dataMigrationPage.step2"),
    t("dataMigrationPage.step3"),
    t("dataMigrationPage.step4"),
  ];
        ...
        <CardContent className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              ...
              JSON ({t("dataMigrationPage.jsonFullData")})
            </Button>
            <Button
              variant="outline"
              className="gap-2"
            ...
            >
              ...
              CSV ({t("dataMigrationPage.csvCustomers")})
            </Button>
            <Button
              variant="outline"
              className="gap-2"
            ...
            >
              ...
              PDF ({t("dataMigrationPage.pdfPrint")})
            </Button>
          </div>
            ...
            {IMPORT_SOURCES.map((src) => (
              ...
              >
                ...
                <p className="text-sm font-medium">
                  {src.id === "salonappy" ? t("dataMigrationPage.sourceBookingApp") : src.id === "arvengo" ? t("dataMigrationPage.sourceOtherSoftware") : "Excel / CSV"}
                </p>
                {selectedSource === src.id && (
            ...
            <div className="space-y-3">
              <div className="p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                <p className="text-xs text-amber-800 dark:text-amber-300">
                  <strong>{t("dataMigrationPage.noteLabel")}:</strong>{" "}
                  {selectedSource === "salonappy" && t("dataMigrationPage.noteSalonappy")}
                  {selectedSource === "arvengo" && t("dataMigrationPage.noteArvengo")}
                  {selectedSource === "excel" && t("dataMigrationPage.noteExcel")}
                </p>
              </div>
